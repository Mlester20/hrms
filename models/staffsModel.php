<?php
    ...
    class staffsModel extends BaseModel {
        ...
        public function getAllStaffs() {
            try {
                $query = "SELECT * FROM " . $this->staffs . " ORDER BY staff_id DESC";
        ...
        public function addStaff($data) {
            try {
                ...
                $query = "INSERT INTO " . $this->staffs . " (name, position, address, shift_type, phone_number, email, password) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $this->con->prepare($query);
        ...
        public function updateStaff($data) {
            try {
                ...
                if (!empty($password)) {
                    ...
                    $query = "UPDATE " . $this->staffs . " SET name = ?, position = ?, address = ?, shift_type = ?, phone_number = ?, email = ?, password = ? WHERE staff_id = ?";
                    $stmt = $this->con->prepare($query);
                    $stmt->bind_param("sssssssi", $name, $position, $address, $shift_type, $phone_number, $email, $password, $staff_id);
                } else {
                    $query = "UPDATE " . $this->staffs . " SET name = ?, position = ?, address = ?, shift_type = ?, phone_number = ?, email = ? WHERE staff_id = ?";
                    $stmt = $this->con->prepare($query);
        ...
        public function deleteStaff($staff_id) {
            try {
                $query = "DELETE FROM " . $this->staffs . " WHERE staff_id = ?";
                $stmt = $this->con->prepare($query);
