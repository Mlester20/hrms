...
class dashboardModel{
    ...
    public function getRevenueData($con, $period = 'year') {
        try {
            switch($period) {
                case 'week':
                    // Last 7 days
                ...
                case 'month':
                    ...
                    $query = "SELECT DAY(created_at) as day, SUM(total_price) as revenue 
                            FROM bookings 
                            WHERE YEAR(created_at) = YEAR(CURDATE()) 
            ...
            if ($period == 'week') {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($index !== false) {
                        $data[$index] = round((float)$row['revenue'], 2); 
                    }
                }
            } elseif ($period == 'month') {
                ...
                while ($row = $result->fetch_assoc()) {
                    $day = (int)$row['day'];
                    $data[$day - 1] = round((float)$row['revenue'], 2);
                }
            } else {
                // Year - 12 months
                $months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                ...
                while ($row = $result->fetch_assoc()) {
                    $data[$row['month'] - 1] = round((float)$row['revenue'], 2); 
                }
            }
